...
def get_passages_from_unlabeled_txt(filename):
    ...
    list_of_passages = [
        passage.strip() for passage in list_of_passages
...
def calculate_average_accuracy(
    deceptive_accuracy, deceptive_predictions, truthful_accuracy, truthful_predictions
):
    average_accuracy = (
        deceptive_accuracy * len(deceptive_predictions)
        + truthful_accuracy * len(truthful_predictions)
    ) / (len(deceptive_predictions) + len(truthful_predictions))
...
if __name__ == "__main__":
    ...
    # Predict
    perplex_unigram_truthful_tprobs = compute_perplex_unigram(
        transformed_valid_reviews_truthful, unigram_prob_dict_truthful
    )
    perplex_unigram_truthful_dprobs = compute_perplex_unigram(
        transformed_valid_reviews_truthful, unigram_prob_dict_deceptive
    )
    perplex_unigram_deceptive_tprobs = compute_perplex_unigram(
        transformed_valid_reviews_deceptive, unigram_prob_dict_truthful
    )
    perplex_unigram_deceptive_dprobs = compute_perplex_unigram(
        transformed_valid_reviews_deceptive, unigram_prob_dict_deceptive
    )
    perplex_bigram_truthful_tprobs = compute_perplex_bigram(
        transformed_valid_reviews_truthful,
        unigram_prob_dict_truthful,
        bigram_prob_dict_truthful,
    )
    perplex_bigram_truthful_dprobs = compute_perplex_bigram(
        transformed_valid_reviews_truthful,
        unigram_prob_dict_deceptive,
        bigram_prob_dict_deceptive,
    )
    perplex_bigram_deceptive_tprobs = compute_perplex_bigram(
        transformed_valid_reviews_deceptive,
        unigram_prob_dict_truthful,
        bigram_prob_dict_truthful,
    )
    perplex_bigram_deceptive_dprobs = compute_perplex_bigram(
        transformed_valid_reviews_deceptive,
        unigram_prob_dict_deceptive,
        bigram_prob_dict_deceptive,
    )

    unigram_predictions_truthful = make_predictions(
        perplex_unigram_truthful_dprobs, perplex_unigram_truthful_tprobs
    )
    unigram_predictions_deceptive = make_predictions(
        perplex_unigram_deceptive_dprobs, perplex_unigram_deceptive_tprobs
    )
    bigram_predictions_truthful = make_predictions(
        perplex_bigram_truthful_dprobs, perplex_bigram_truthful_tprobs
    )
    bigram_predictions_deceptive = make_predictions(
        perplex_bigram_deceptive_dprobs, perplex_bigram_deceptive_tprobs
    )

    unigram_accuracy_truthful = eval_preditions(
        unigram_predictions_truthful, "truthful"
    )
    unigram_accuracy_deceptive = eval_preditions(
        unigram_predictions_deceptive, "deceptive"
    )
    unigram_accuracy = calculate_average_accuracy(
        unigram_accuracy_deceptive,
        unigram_predictions_deceptive,
        unigram_accuracy_truthful,
        unigram_predictions_truthful,
    )
    bigram_accuracy_truthful = eval_preditions(bigram_predictions_truthful, "truthful")
    bigram_accuracy_deceptive = eval_preditions(
        bigram_predictions_deceptive, "deceptive"
    )
    bigram_accuracy = calculate_average_accuracy(
        bigram_accuracy_deceptive,
        bigram_predictions_deceptive,
        bigram_accuracy_truthful,
        bigram_predictions_truthful,
    )
